...
Route::get('/download-app', function () {
    if (!session()->has('role_id')) {
        return redirect()->route('login')->withErrors(['login' => 'Silakan login dengan akun yang memiliki hak akses untuk mengunduh aplikasi ini.']);
    }
    
    $roleId = (int) session('role_id');
    // Role 1 = Petani/Kelompok Tani, 2 = Petugas, 5 = Brigade Pangan
    if (in_array($roleId, [1, 2, 5])) {
        $filePath = storage_path('app/sipetani-mobile.apk');
        if (file_exists($filePath)) {
            return response()->download($filePath, 'SiPetani-Mobile.apk');
        }
        abort(404, 'File aplikasi tidak ditemukan.');
    }
    
    return redirect()->route('login')->withErrors(['login' => 'Akun Anda tidak memiliki hak akses (role) untuk mengunduh aplikasi mobile ini.']);
})->name('download.app');
