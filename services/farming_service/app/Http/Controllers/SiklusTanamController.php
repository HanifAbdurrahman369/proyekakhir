...
class SiklusTanamController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Dihapus: Pengecekan lahan_id & status_aktif AKTIF agar bisa lapor tanam kapan saja
    ...
    public function storeLaporPanen(Request $request)
    {
        ...
        // Dihapus: Pengecekan lapor panen yang sedang/telah diverifikasi agar bebas akses
    ...
    public function destroy(Request $request, $id)
    {
        ...
        // Dihapus: Pengecekan panen untuk destroy agar bisa menghapus tanpa block
    ...
    public function approve(Request $request, $id)
    {
        ...
        $result = DB::transaction(function () use ($id, $petugasId) {
            $panen = LaporPanen::with('siklusTanam')->where('id', $id)->lockForUpdate()->first();
            if (!$panen) {
            ...
            if (!$this->petugasBolehVerifikasiLahan($petugasId, (int) $panen->siklusTanam->lahan_id)) {
            ...
            $this->sinkronkanInfoLahanDariPanenTerakhir((int) $panen->siklusTanam->lahan_id);
    ...
    public function reject(Request $request, $id)
    {
        ...
        $panen = LaporPanen::with('siklusTanam')->find($id);
        if (!$panen) {
    ...
    private function formatHasilPanen($row): array
    {
        return [
            ...
            'luas_lahan_hektar' => (float) ($row->lahan_luas_lahan_hektar ?? $row->luas_lahan_ha ?? 0),
            'luas_tanam_hektar' => (float) ($row->tanam_luas_tanam_hektar ?? $row->lahan_luas_lahan_hektar ?? 0),
            'produktivitas_pengajuan_ton_ha' => (float) (($row->tanam_luas_tanam_hektar ?? $row->lahan_luas_lahan_hektar ?? 0) > 0 ? round($row->hasil_panen_ton / ($row->tanam_luas_tanam_hektar ?? $row->lahan_luas_lahan_hektar), 2) : 0),
    ...
    {
        $panen = LaporPanen::with('siklusTanam.lahan')->whereHas('siklusTanam', fn($q) => $q->where('lahan_id', $lahanId))
        ...
        $luasTanam = (float) ($panen->siklusTanam->luas_tanam_hektar ?: ($panen->siklusTanam->lahan->luas_lahan_hektar ?? 1));
        DB::table('lahan_sawah')->where('id', $lahanId)->update([
            'hasil_panen_ton' => $panen->hasil_panen_ton,
            'luas_tanam_hektar' => $panen->siklusTanam->luas_tanam_hektar ?: ($panen->siklusTanam->lahan->luas_lahan_hektar ?? 0),
            'produktivitas_ton_ha' => $luasTanam > 0 ? round($panen->hasil_panen_ton / $luasTanam, 2) : 0,
