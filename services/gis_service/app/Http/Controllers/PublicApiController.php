    ...
    private function panenDiterimaPerLahanQuery()
    {
        if (Schema::hasTable('panen_padi')) {
            return DB::table('panen_padi as rp')
                ...
                ->select('tp.lahan_id', DB::raw('COALESCE(SUM(rp.hasil_panen_ton),0) as total_panen'))
        ...
        if (Schema::hasTable('panen_padi')) {
            return DB::table('panen_padi as rp')
                ...
                ->select(
                    ...
                    DB::raw('ROUND(SUM(COALESCE(tp.luas_tanam_hektar, ls.luas_lahan_hektar)), 2) as total_luas_panen'),
                    DB::raw('CASE WHEN SUM(COALESCE(tp.luas_tanam_hektar, ls.luas_lahan_hektar)) > 0 THEN ROUND(SUM(rp.hasil_panen_ton) / SUM(COALESCE(tp.luas_tanam_hektar, ls.luas_lahan_hektar)), 2) ELSE 0 END as produktivitas_ton_ha')
                )
                ->groupBy('kecamatan.nama_kecamatan')
                ->orderBy('kecamatan.nama_kecamatan')
                ->get()
        ...
        if (Schema::hasTable('panen_padi')) {
            $panenRows = DB::table('panen_padi as rp')
                ...
                ->select(
                    ...
                    DB::raw('ROUND(COALESCE(SUM(COALESCE(tp.luas_tanam_hektar, ls.luas_lahan_hektar)),0), 2) as total_luas_panen_ha'),
                    DB::raw('CASE WHEN SUM(COALESCE(rp.hasil_panen_ton,0)) > 0 AND SUM(COALESCE(tp.luas_tanam_hektar, ls.luas_lahan_hektar)) > 0 THEN ROUND(SUM(rp.hasil_panen_ton) / SUM(COALESCE(tp.luas_tanam_hektar, ls.luas_lahan_hektar)), 2) ELSE 0 END as produktivitas_ton_ha')
                )
